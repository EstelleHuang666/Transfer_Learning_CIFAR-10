...
def load_pool3_data():
    ...
    X_test_file = 'X_test_20170609-16:31:21.npy'
    y_test_file = 'y_test_20170609-16:31:21.npy'
    X_train_file = 'X_train_20170609-16:31:21.npy'
    y_train_file = 'y_train_20170609-16:31:21.npy'
...
BOTTLENECK_TENSOR_TRAIN_NAME = "bottlerneck_train"
BOTTLENECK_TENSOR_TEST_NAME = "bottlerneck_test"
...
# https://github.com/tensorflow/tensorflow/blob/master/tensorflow/examples/image_retraining/retrain.py
def add_final_training_ops(graph, bottleneck_tensor, ground_truth_placeholder, 
    ...
    tf.nn.softmax(logits, name="output_tensor_name")
...
# https://github.com/tensorflow/tensorflow/blob/master/tensorflow/examples/image_retraining/retrain.py
#def add_evaluation_step(graph, bottleneck_tensor, ground_truth_tensor,
#                        layer_weights, layer_biases):
def add_evaluation_step(graph, bottleneck_tensor, ground_truth_tensor, layer_weights, layer_biases):
    ...
    Returns:
      Nothing.
    """
    result_tensor = tf.nn.softmax(tf.matmul(bottleneck_tensor, layer_weights)+layer_biases)
...
def do_train(sess,X_input, Y_input, X_validation, Y_validation):
    ...
    bottleneck_tensor_train = tf.placeholder(tf.float32, 
                                             [mini_batch_size, BOTTLENECK_TENSOR_SIZE], 
                                             name = BOTTLENECK_TENSOR_TRAIN_NAME)
    ground_truth_tensor_train = tf.placeholder(tf.float32,
                                         [mini_batch_size, len(classes)],
                                         name = GROUND_TRUTH_TENSOR_TRAIN)
    
    layer_weights = tf.Variable(tf.truncated_normal([BOTTLENECK_TENSOR_SIZE, len(classes)], 
        stddev=0.001), name = LAYER_WEIGHTS_NAME)
    layer_biases = tf.Variable(tf.zeros([len(classes)]), name = LAYER_BIASES_NAME)
    
    # Initialize tensorflow variable for the validating process
    bottleneck_tensor_validation = tf.placeholder(tf.float32, 
                                             [mini_batch_size, BOTTLENECK_TENSOR_SIZE])
    ground_truth_tensor_validation = tf.placeholder(tf.float32,
                                         [mini_batch_size, len(classes)])    
    ...
    evaluation_step_validation = add_evaluation_step(
            graph, 
            bottleneck_tensor_validation, 
            ground_truth_tensor_validation,
            layer_weights,
            layer_biases
            )
    evaluation_step_test = add_evaluation_step(
            graph, 
    ...
    for epoch in range(epocs):
        ...
        for Xi, Yi in iterate_mini_batches(shuffledX, shuffledY, mini_batch_size):
            _, cross_entropy_value = sess.run([train_step, cross_entropy],
                     feed_dict={bottleneck_tensor_train: Xi,
                                ground_truth_tensor_train: Yi})
            # Every so often, print out how well the graph is training.
            is_last_step = (i + 1 == FLAGS.how_many_training_steps)
            if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
                train_accuracy = sess.run(
                  evaluation_step_validation,
                  feed_dict={bottleneck_tensor_validation: Xi,
                             ground_truth_tensor_validation: Yi})
                validation_accuracy = sess.run(
                  evaluation_step_test,
                  feed_dict={bottleneck_tensor_test: X_validation,
                             ground_truth_tensor_test: y_one_hot_validation})
    ...
    test_accuracy = sess.run(
        evaluation_step_test,
        feed_dict={bottleneck_tensor_test: X_test_pool3,
                   ground_truth_tensor_test: encode_one_hot(len(classes), y_test_pool3)})
